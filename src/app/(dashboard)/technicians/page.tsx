...
export default function TechniciansPage() {
  const available = MOCK_TECHNICIANS.filter(t => t.status === "available").length;

  return (
    <div className="flex flex-col min-h-screen">
      <TopBar title="Technicians" subtitle={`${MOCK_TECHNICIANS.length} total · ${available} available`} />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5 text-sm">
            {[
              ...
              { label: "On Job",    count: MOCK_TECHNICIANS.filter(t => t.status === "on_job").length, cls: "text-primary" },
              { label: "En Route",  count: MOCK_TECHNICIANS.filter(t => t.status === "on_the_way").length, cls: "text-c-teal" },
            ].map(({ label, count, cls }) => (
          ...
          {MOCK_TECHNICIANS.map(tech => (
            <div key={tech.id} className="rounded-lg border border-border bg-card p-5 space-y-4">
              <div className="flex items-start justify-between">
                ...
                <span className={cn("text-xs font-medium px-1.5 py-0.5 rounded border", TECH_STATUS_STYLE[tech.status])}>
