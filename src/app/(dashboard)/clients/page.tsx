...
export default function ClientsPage() {
  return (
    <div className="flex flex-col min-h-screen">
      <TopBar title="Clients" subtitle={`${MOCK_CLIENTS.length} clients`} />

      <div className="flex-1 px-6 py-6 space-y-5">

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 text-sm">
            <span className="text-2xl font-semibold text-foreground">{MOCK_CLIENTS.length}</span>
            <span className="text-muted-foreground">total clients ·</span>
            <span className="text-c-success">{MOCK_CLIENTS.filter(c => c.status === "active").length} active</span>
          </div>
              ...
              <div className="rounded-lg border border-border bg-card p-5 hover:border-border/80 hover:bg-muted/20 transition-colors group">
                ...
                <div className="flex items-center gap-4 text-xs border-t border-border pt-3">
                  <div>
                    ...
                    <p className="font-semibold text-foreground">{client.sites}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Jobs</p>
                    <p className="font-semibold text-foreground">{client.jobs}</p>
                  </div>
