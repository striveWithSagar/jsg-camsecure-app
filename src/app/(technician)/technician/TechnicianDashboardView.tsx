...
const ACTIVE_STATUSES = new Set(["in_progress", "started", "on_the_way"]);
...
export function TechnicianDashboardView({
  ...
  const todayJobs     = jobs.filter(j => j.scheduled.startsWith("Today"));
  ...
  return (
    <div className="space-y-6">
              ...
              <Link key={job.id} href={`/technician/jobs/${job.id}`}>
                <div className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {" · "}{job.site} · {job.scheduled.replace("Today ", "")}
                  </p>
                  </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Assigned",    value: jobs.filter(j => j.status !== "completed" && j.status !== "cancelled").length, color: "text-foreground" },
          { label: "Completed",   value: jobs.filter(j => j.status === "completed").length,   color: "text-c-success" },
          { label: "Needs Parts", value: jobs.filter(j => j.status === "needs_parts").length,  color: "text-c-warning" },
